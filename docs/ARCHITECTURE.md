...
│   ├── vector_store.py             # pgvector tables, HNSW indexes, query/ingest (real data + document chunks)
│   ├── document_ingestion.py       # PDF/DOCX/TXT parser and text chunker (800-char chunks, 150 overlap)
│   ├── pipeline/
│   │   ├── state.py                # compute_source_quality() descriptor
│   │   ├── nodes.py                # All LangGraph node functions (Tavily web search, retrieve_uploaded)
...
│   ├── constants.js                # API_BASE URL (reads REACT_APP_API_URL)
│   └── components/
│       ├── Header.jsx
...
│   ├── generate_data.py            # Synthetic CSV generator (fallback only, not ingested by default)
│   ├── ingest_documents.py         # Batch script: parse+chunk files in documents/, move to processed/
│   ├── documents/                  # Drop PDF/DOCX/TXT files here to ingest
│   │   └── processed/              # Files moved here after successful ingestion
│   ├── medical_pubmed.csv          # 786 real PubMed abstracts (generated by fetch_real_data.py)
│   ├── medical_fda_labels.csv      # 23 real FDA drug labels (generated by fetch_real_data.py)
│   ├── medical_q_n_a.csv           # Synthetic Q&A dataset (not ingested unless real data absent)
│   └── medical_device_manuals_dataset.csv  # Synthetic device dataset (not ingested unless real data absent)
│
├── migrations/                     # Alembic database migrations
│   ├── env.py                      # Alembic environment (reads DATABASE_URL)
│   ├── script.py.mako              # Migration template
│   └── versions/
│       ├── 20240101_0001_initial_schema.py  # Initial tables + indexes (with downgrade())
│       └── 20260409_0002_document_chunks.py # document_chunks table + HNSW + filename index
│
├── scripts/                        # Utility scripts (not part of the app)
│   ├── smoke_test.py               # Manual CLI smoke tester (was test_system.py at project root)
...
│   ├── test_document_ingestion.py  # Unit tests for document parsing, chunking, and ID generation
...
├── Dockerfile                      # Backend container (entrypoint: migrations + uvicorn)
├── Dockerfile.frontend             # Frontend container (multi-stage: Node 24 build → nginx 1.25)
├── nginx.conf                      # nginx: SPA routing + /api/* proxy + SSE support
├── docker-compose.yml              # Local full-stack: db + backend + frontend
├── .dockerignore                   # Excludes .env, node_modules, .venv, tests/ from build context
...
> **Important:** `LLM_BASE_URL` must be a full URL (e.g. `https://...`) or left empty/unset. An empty string is treated as unset — the official OpenAI endpoint is used.
...
Browser
  ...
  [frontend] nginx:1.25
        ...
        └── /api/* → proxy_pass http://backend:8000 (SSE buffering off)
                          ↓
                   [backend] uvicorn:8000
                        ├── alembic upgrade head  (on startup via docker-entrypoint.sh)
                        └── FastAPI app
                              ↓
...
The corpus consists entirely of real medical data fetched from public APIs.

| CSV | Source | Records | Priority |
|-----|--------|---------|---------|
| `data/medical_pubmed.csv` | PubMed E-utilities API (free) | 786 | 1st — real clinical abstracts |
| `data/medical_fda_labels.csv` | openFDA drug labels API (free) | 23 | 1st — real FDA drug label text |
| `data/medical_q_n_a.csv` | Synthetic generator | 1,000 | Fallback only (not ingested by default) |
| `data/medical_device_manuals_dataset.csv` | Synthetic generator | 1,000 | Fallback only (not ingested by default) |

Run `python data/fetch_real_data.py` to regenerate the real data CSVs. Synthetic rows are tagged with `SYNTHETIC — do not rely on for clinical decisions` in the `source_label` metadata field if ingested.

## Database Design

All persistence is in **PostgreSQL + pgvector**. Four tables:

| Table | Purpose | Key Columns |
|-------|---------|------------|
| `medical_qna` | Q&A vector store | `id TEXT`, `content TEXT`, `embedding vector(1536)`, `metadata JSONB` |
| `medical_device` | Device / drug label vector store | `id TEXT`, `content TEXT`, `embedding vector(1536)`, `metadata JSONB` |
| `document_chunks` | Uploaded document chunks | `id TEXT`, `content TEXT`, `embedding vector(1536)`, `metadata JSONB`, `filename TEXT`, `chunk_index INT`, `uploaded_at TIMESTAMP` |
...
Schema is managed by **Alembic** migrations (`migrations/`). The backend runs `alembic upgrade head` automatically on every startup via `scripts/docker-entrypoint.sh`.
...
| Uploaded Document | `uploaded_document` | Uploaded document |
